...
function CreateScreen({ onCreated }: { onCreated: (c: InstructorCreds) => void }) {
  ...
  async function launch(deckId?: string) {
    ...
    setErr('');
    try {
      const body: CreateClassroomRequest = deckId ? { deckId, mode, settings } : { mode, settings };
...
function Console({ creds, onReset }: { creds: InstructorCreds; onReset: () => void }) {
  ...
  const nextStep = (): Step => {
    if (activeHere) {
      ...
      if (openAct?.type === 'poll') {
        // 타이머 투표 진행 중 → 지금 마감. 결과가 아직 숨겨져 있으면 → 결과 공개 (after_close 정책이면 응답 마감 겸함)
        if (pollState?.endsAt && !pollState.closed && !activeHere.closed)
          return { label: `⏹ 지금 마감 · ${pollTotal}명 응답`, primary: true, run: () => live.socket.emit('instructor:pollClose') };
        if (!activeHere.revealResults)
          return { label: `📢 결과 공개 · ${pollTotal}명 응답`, primary: true, run: () => live.socket.emit('instructor:revealResults') };
      }
        ...
        <section className={[focusMode ? '' : 'lg:col-span-2', 'flex flex-col gap-3 overflow-hidden'].join(' ')}>
          ...
          {activeHere && (
            <div className="rounded-xl bg-surface px-3 py-2 text-center text-sm ring-1 ring-hairline shadow-card">
              ...
              {openAct?.type === 'poll' && (
                <div className="space-y-2" data-testid="poll-status" data-poll-closed={!!activeHere.closed || (pollState?.closed ?? false)} data-poll-revealed={activeHere.revealResults}>
                  ...
                  {pollState?.endsAt && !pollState.closed && !pollState.revealed && (
