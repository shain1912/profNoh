    ...
    <div className="flex h-full flex-col p-4 overflow-y-auto">
      ...
      {loading && (
        <div className="mt-6 flex-1 flex flex-col items-center justify-center space-y-3 py-10">
          ...
          <p className="text-sm text-muted animate-pulse">두 캐릭터가 열심히 설명하는 중입니다...</p>
        </div>
      )}
      ...
      {!loading && result && (
        <div className="mt-6 flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in pb-6">
          {/* 캐릭터 A 설명 */}
          <div className="p-5 rounded-2xl bg-brand/5 border border-brand/20 flex flex-col justify-between hover:border-brand/40 transition-all duration-300 shadow-card">
            <div>
              <span className="inline-block px-2.5 py-1 rounded-full bg-brand/10 text-brand text-xs font-bold mb-3">
          ...
          {/* 캐릭터 B 설명 */}
          <div className="p-5 rounded-2xl bg-warn/5 border border-warn/20 flex flex-col justify-between hover:border-warn/40 transition-all duration-300 shadow-card">
            <div>
              <span className="inline-block px-2.5 py-1 rounded-full bg-warn/10 text-warn text-xs font-bold mb-3">
            ...
            <div className="text-right mt-4 text-xs text-warn/70 font-semibold">
              # {topic} # 색다른 관점
            </div>
          </div>
        </div>
