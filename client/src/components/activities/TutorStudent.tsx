...
주어진 문제를 해결하다가 막힌 부분이나 막 작성한 풀이/코드를 입력해주시면, 스스로 해답을 찾을 수 있도록 힌트를 드릴게요! 💡`,
  ...
  const subjectBadges = {
    math: { label: '수학 🧮', color: 'bg-down/10 text-down border-down/30' },
    coding: { label: '코딩 💻', color: 'bg-up/10 text-up border-up/30' },
    general: { label: '일반 학습 💡', color: 'bg-info/10 text-info border-info/30' },
  };
  ...
  return (
    <div className="flex h-full flex-col p-4">
      ...
      {/* 문제 설명 카드 */}
      <div className="mt-3 rounded-xl p-3 bg-surface-2 ring-1 ring-hairline text-sm">
        <div className="font-extrabold text-muted mb-1 flex items-center gap-1.5">
          <span>📝 실습 과제 설명</span>
        </div>
      ...
      <div className="mt-3 flex-1 space-y-3 overflow-y-auto rounded-xl bg-surface-2 p-3 ring-1 ring-hairline">
        ...
        {loading && (
          <div className="text-left text-muted-2 text-xs flex items-center gap-2 animate-pulse">
